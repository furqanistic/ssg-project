...
const NavbarDropdown = ({ item, isOpen }) => {
  return (
    <div
      className={`absolute left-1/2 top-[calc(100%+8px)] z-30 -translate-x-1/2 transition-all duration-300 ease-out origin-top ${
        isOpen ? 'scale-100 opacity-100 visible translate-y-0' : 'scale-95 opacity-0 invisible -translate-y-2'
      }`}
    >
      <div
        className={`grid gap-x-8 gap-y-6 rounded-2xl border border-white/15 bg-white/95 px-6 py-6 text-left shadow-[0_8px_32px_-16px_rgba(0,0,0,0.3)] backdrop-blur-xl ${item.panelClassName}`}
      >
        {item.sections.map((section, index) => (
          <div key={`${item.label}-${index}`} className='min-w-0'>
            {section.heading ? (
              <p className='mb-3 text-[10px] font-medium uppercase tracking-[0.22em] text-[#6b7485]/70'>
                {section.heading}
              </p>
            ) : null}
              ...
              {section.links.filter((link) => link.label).map((link) => {
                ...
                return (
                  <Component
                    ...
                    to={link.to}
                    href={link.href}
                    className='group flex items-center text-[15px] font-medium text-[#1d2738] transition-colors duration-300 hover:text-[#f09816]'
                  >
                    {link.label}
                  </Component>
                )
...
const NavbarSection = () => {
  ...
  return (
    <header
      className='fixed inset-x-0 top-0 z-50 border-b border-white/8 bg-[#1e3a8a]'
      onMouseLeave={() => {
        setOpenItem(null)
    ...
    >
      <div className='border-b border-white/5'>
        <div className='mx-auto flex h-10 w-full items-center justify-start gap-3 px-3 text-[11px] font-medium tracking-wide text-white/70 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:justify-end md:gap-4 md:px-4 md:text-[11px] lg:px-8'>
          <div className='relative'>
            <button
              type='button'
              onClick={() => setIsContactPopupOpen((prev) => !prev)}
              className='inline-flex items-center gap-1.5 rounded-full border border-white/12 bg-white/6 px-3 py-1 text-[11px] font-medium text-white/75 transition-all duration-500 hover:bg-white/10'
            >
              <PhoneCall className='h-3.5 w-3.5' />
              <span>+49 15567 277478</span>
              <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-300 ${isContactPopupOpen ? 'rotate-180' : ''}`} />
            </button>
            <div
              className={`absolute left-1/2 top-[calc(100%+6px)] z-40 w-[170px] -translate-x-1/2 rounded-xl border border-black/8 bg-white/95 p-2 shadow-[0_8px_32px_-16px_rgba(0,0,0,0.3)] transition-all duration-300 md:left-0 md:translate-x-0 ${
                isContactPopupOpen ? 'visible scale-100 opacity-100' : 'invisible scale-95 opacity-0'
              }`}
            >
              <a
                href='tel:+4915567277478'
                className='flex items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[12px] font-medium text-[#1d2738]/85 transition-colors duration-300 hover:bg-[#1e3a8a]/8'
              >
                <PhoneCall className='h-3.5 w-3.5 text-[#1e3a8a]/70' />
                Call
              </a>
              <a
                ...
                target='_blank'
                rel='noreferrer'
                className='mt-1 flex items-center gap-2 rounded-lg px-2.5 py-2 text-left text-[12px] font-medium text-[#1d2738]/85 transition-colors duration-300 hover:bg-[#1e3a8a]/8'
              >
                <WhatsAppIcon className='h-3.5 w-3.5 text-[#1e3a8a]/70' />
                WhatsApp
              </a>
            </div>
          </div>
      ...
      <div className='mx-auto flex h-[72px] w-full items-center justify-between px-4 md:h-16 md:px-6 lg:px-8'>
        <Link to='/' className='flex items-center gap-3 md:gap-4'>
          ...
          <div className='flex max-w-[210px] flex-col justify-center md:max-w-none'>
            <span className='text-[13px] font-medium leading-tight tracking-tight text-white sm:text-[14px] md:text-[18px]'>
              {t('navbar.brand.name')}
            </span>
            <span className='text-[10px] font-medium uppercase tracking-[0.15em] text-white/50 sm:text-[11px] md:text-[11px]'>
        ...
        <button
          ...
          className='inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-white/75 transition-colors duration-500 hover:bg-white/10 md:hidden'
        ...
        <nav className='hidden items-center gap-1 md:flex'>
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <button
                  className='flex h-9 items-center gap-1.5 rounded-full px-3.5 text-[13px] font-medium text-white/80 transition-colors duration-300 hover:text-white'
                  type='button'
        ...
        <div className='hidden items-center gap-4 lg:flex'>
          <div className='flex items-center rounded-full border border-white/12 bg-white/6 p-1'>
            {['en', 'de'].map((lang) => (
              <button
                key={lang}
                type='button'
                onClick={() => i18n.changeLanguage(lang)}
                className={`flex h-8 items-center justify-center rounded-full px-4 text-[11px] font-medium uppercase tracking-wider transition-all duration-500 ${
                  i18n.language === lang
          ...
          <Link
            ...
            className='text-[13px] font-medium text-white/60 transition-colors duration-500 hover:text-white/90'
          >
            {isLoggedIn ? t('common.actions.dashboard') : t('common.actions.login')}
          </Link>
          <Link
            to='/donate'
            className='inline-flex h-9 items-center justify-center rounded-full bg-[#f09816] px-5 text-[13px] font-medium text-white transition-all duration-500 hover:bg-[#f1a52e] active:scale-[0.97]'
          >
      ...
      <div
        className={`absolute inset-x-0 top-full border-t border-white/8 bg-[#1e3a8a] px-4 pb-4 pt-3 shadow-[0_8px_32px_-16px_rgba(0,0,0,0.4)] md:hidden ${
      ...
      >
        <nav className='flex max-h-[calc(100dvh-132px)] flex-col overflow-y-auto pb-2'>
          {navItems.map((item) => {
            ...
            return (
              <div key={`mobile-${item.label}`} className='border-b border-white/8 py-1.5'>
                <button
                  type='button'
                  onClick={() => setMobileOpenItem(isItemOpen ? null : item.label)}
                  className='flex w-full items-center justify-between py-2 text-left text-[15px] font-medium text-white/85'
                >
                ...
                {isItemOpen ? (
                  <div className='mt-1 flex flex-col gap-4 rounded-xl border border-white/8 bg-white/[0.04] px-2 pb-3 pt-2'>
                          ...
                          {section.links.filter((link) => link.label).map((link) => {
                            ...
                            return (
                              <Component
                                ...
                                className='rounded-md px-1 py-1 text-[14px] font-medium text-white/85 transition-colors duration-300 hover:text-[#f6ab3c]'
                              >
                                {link.label}
                              </Component>
                            )
        ...
        <div className='mt-4 flex items-center justify-between gap-2.5 rounded-2xl border border-white/8 bg-white/[0.04] p-2.5'>
          ...
          <Link
            ...
            className='text-[13px] font-medium text-white/70 transition-colors duration-500 hover:text-white/90'
          >
            {isLoggedIn ? t('common.actions.dashboard') : t('common.actions.login')}
          </Link>
          ...
          <Link
            to='/donate'
            onClick={() => setIsMobileMenuOpen(false)}
            className='inline-flex h-9 items-center justify-center rounded-full bg-[#f09816] px-4 text-[13px] font-medium text-white transition-all duration-500 hover:bg-[#f1a52e]'
          >
            {t('common.actions.donate')}
          </Link>
        </div>
